...
| F-102 | ✅ | `brokers/ibkr_client.py` | **Concurrent `reqPositionsAsync()` calls hang — TWS sends `positionEnd` only once.** When multiple strategies ran reconcile simultaneously, each called `get_positions(force_refresh=True)` → `run_in_executor` → `_manager.fetch_positions()` → `_submit(_do_positions())`. Every `_do_positions()` called `reqPositionsAsync()` which internally waits for TWS to emit a `positionEnd` event. TWS only emits this event once per request cycle — the first coroutine consumed it; subsequent ones waited forever until the 15 s `_submit()` timeout fired raising `concurrent.futures.TimeoutError("")` (empty message). This produced the `"ibkr positions unavailable:"` log with no exception text and `"Task was destroyed but it is pending!"` warnings. **Fix:** Added `_positions_lock: asyncio.Lock` (created alongside `_connect_lock` in `_run_loop()`) to `_IBKRManager`; `_do_positions()` acquires the lock before calling `reqPositionsAsync()`, serialising all concurrent callers so only one position request is in flight at a time. |
