...
class MomentumBreakoutStrategy(BaseStrategy):
    ...
    def generate_signal(
        ...
        **kwargs,
        ...
        # ML gate: BUY uses predict_proba (P(buy)); SHORT uses dedicated short model (G6)
        ...
        ML_VETO_SHORT = 0.35   # P(short_win) below this → suppress SHORT
        ...
        if broke_down and short_score >= SHORT_THRESHOLD:
            if ml_short_prob is not None and ml_short_prob < ML_VETO_SHORT:
                return _hold([f"ML veto: P(short_win)={ml_short_prob:.2f} < {ML_VETO_SHORT} — breakdown unconfirmed by model"])
            ...
            return self._enhance_signal(Signal(
                ...
                timeframe=timeframe, strategy_name=self.name,
                asset_class=self.asset_class, broker=self.broker,
                reasons=(reasons_short if broke_down else reasons) + [f"Regime: {regime_name}"] + ([f"ML P(short)={ml_short_prob:.2f}"] if ml_short_prob is not None else []),
                regime=regime_name,
            ), data, atr_val)
